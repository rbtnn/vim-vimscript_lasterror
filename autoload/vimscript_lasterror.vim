...
function! vimscript_lasterror#exec(q_args) abort
	...
	elseif s:QUICKFIX == a:q_args
		call vimscript_lasterror#exec_quickfix()
	elseif s:LOCLIST == a:q_args
		call vimscript_lasterror#exec_loclist()
	elseif s:MESSAGES == a:q_args
		call vimscript_lasterror#exec_messages()
	else
		call vimscript_lasterror#exec_noargs()
	endif
endfunction

function! vimscript_lasterror#exec_quickfix() abort
...
function! vimscript_lasterror#exec_loclist() abort
	let xs = vimscript_lasterror#parse_messages()
	if !empty(xs)
...
function! vimscript_lasterror#exec_messages() abort
	new
	...
	nnoremap <silent><nowait><buffer><cr>   :<C-u>call vimscript_lasterror#exec_messages_cr()<cr>
endfunction
...
function! vimscript_lasterror#exec_messages_cr() abort
	let err_i = line('.') - 1
	let lines = getbufline(bufnr(), 1, '$')
	call map(lines, { i, x -> s:parse_messages(x) })
	let x = s:find_lnum_and_pos_from_message(lines, err_i)
	if !empty(x)
		if filereadable(get(x, 'filename', ''))
			if s:find_window_by_path(x['filename'])
				execute printf(':%d', x['lnum'])
...
function! s:find_lnum_and_pos_from_message(lines, err_i) abort
	if get(a:lines[(a:err_i)], 'kind', '') == 'message'
		for lnum_i in range(a:err_i - 1, 0, -1)
			if get(a:lines[lnum_i], 'kind', '') == 'lnum'
				for pos_i in range(lnum_i - 1, 0, -1)
					if get(a:lines[pos_i], 'kind') == 'pos'
						let file_or_func = a:lines[pos_i]['pos']
						let lnum = str2nr(a:lines[lnum_i]['lnum'])
						let errormsg = a:lines[(a:err_i)]['message']
